...
### Morning Lights App
This SmartApp turns on selected lights at a specified time and turns them off at sunrise if no turn-off time is set. Different turn-on times can be configured for each day of the week, and they can be randomized within a specified window to simulate manual operation.
...
* Option to turn lights off when mode changes to Away

...
def initialize() {
    ...
    state.appOn = false
    state.scheduled = [turnOn: null, schedTurnOn: null, turnOff: null]
...
def schedTurnOff(onDelayMS, offDate) { //determine turn-off time and schedule the turnOff()
    ...
    if (offDate) {
        ...
        if (offDate > nowDate) {
            ...
            state.scheduled.turnOff = offDate.time
